...
if(loadstuff){
      ...
      write.csv(mcc_tab, "ba*_space_time_info.csv", row.names=F, quote=F)

      # 3. Estimating the HPD region for each time slice
      mcc_tab=read.csv("ba*_space_time_info.csv")
      ...
      reloadborders<-TRUE
      if(reloadborders){
          borders = gadm(country="ZAF", level=1)
          borders2 = gadm(country="ZAF", level=0)
          template_raster = raster("zaf_pd_2020_1km_cropped.tif")
      }
}
...
c2 <- paletteer_c("grDevices::PuBuGn", 10)
...
pdf('ba*_phylogeography.pdf',width=6, height=6.3,bg="white")
dev.off()
...
plot(template_raster, col = custom_palette, colNA = "white", breaks = custom_breaks, box = F, axes = F, legend = T)
...
for (i in seq(dim(mcc_tab)[1], 1)) #reversed sorting; #for (i in 1:dim(mcc_tab)[1]) - ascending
{
#  curvedarrow(cbind(mcc_tab[i,"startLon"],mcc_tab[i,"startLat"]), cbind(mcc_tab[i,"endLon"],mcc_tab[i,"endLat"]), arr.length=0.2,
#              arr.width=0.1, lwd=2, lty=1, lcol="grey20", arr.col=endYears_colours[i], arr.pos=1, curve=0.3, dr=NA, endhead=F) #lwd=4*1.1
  curvedarrow(cbind(mcc_tab[i,"startLon"],mcc_tab[i,"startLat"]), cbind(mcc_tab[i,"endLon"],mcc_tab[i,"endLat"]), arr.length=0.3,
              arr.width=0.2, lwd=4, lty=1, lcol=endYears_colours[i], arr.col=endYears_colours[i], arr.pos=1, curve=0.3, dr=NA, endhead=F) #lwd=3.5*1.1
}
for (i in dim(mcc_tab)[1]:1)
{
  ...
  if (i == 1)
  {
    ...
    #points(xs, ys, pch=1, col="gray20", cex=ptsize)
  }
  points(xe, ye, pch=21, col="gray20", bg=endYears_colours[i], cex=.6)
  #points(xe, ye, pch=1, col="gray20", cex=ptsize)
}
